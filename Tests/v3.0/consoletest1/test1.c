...
VOID WINAPI test_proc(VOID)
{
	...
	console_set_title(TEXT("Console Test 1"));

	while(n_events_remaining >= 0)
	{
		__SPRINTF(textbuf, TEXTBUF_SIZE_CHARS, TEXT("%d keydown events remaining\r\n"), n_events_remaining);
		console_stdout_write(textbuf);
		Sleep(250);
		if(console_get_key_pressed())
		{
			console_stdout_write(TEXT("Key pressed!!!\r\n"));
			n_events_remaining--;
		}
	}
