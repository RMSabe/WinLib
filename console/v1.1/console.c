...
VOID *_console_on_key_event = NULL;
VOID *_console_on_mouse_event = NULL;
VOID *_console_on_window_buffer_size_event = NULL;
...
VOID WINAPI console_deinit(VOID)
{
	...
	_console_on_window_buffer_size_event = NULL;
...
BOOL WINAPI console_manage_input_events(VOID)
{
	...
	for(n_event = 0u; n_event < n_events; n_event++)
	{
		...
		{
			case KEY_EVENT:
				if(_console_on_key_event != NULL) (*((VOID WINAPI (*)(KEY_EVENT_RECORD*)) _console_on_key_event))((KEY_EVENT_RECORD*) p_event);
				break;

			case MOUSE_EVENT:
				if(_console_on_mouse_event != NULL) (*((VOID WINAPI (*)(MOUSE_EVENT_RECORD*)) _console_on_mouse_event))((MOUSE_EVENT_RECORD*) p_event);
				break;

			case WINDOW_BUFFER_SIZE_EVENT:
				if(_console_on_window_buffer_size_event != NULL) (*((VOID WINAPI (*)(WINDOW_BUFFER_SIZE_RECORD*)) _console_on_window_buffer_size_event))((WINDOW_BUFFER_SIZE_RECORD*) p_event);
				break;
		}
	}
